...
def initialize(job):
    ...
    with job:
        ...
        den = job.statepoint()['den']
        # Load in mol2 files as mb.Compound
        cation = mb.load(wd+'/util/emim.mol2')
        cation.name = 'emim'
        anion = mb.load(wd + '/util/' + str(job.statepoint()['anion']) + '.mol2')
        anion.name = job.statepoint()['anion']
        
        fluor = wd+'/util/lopes_fluor.xml'
        lopes_fluor = Forcefield(fluor)
        lopes = Forcefield(wd+'/util/lopes.xml')
        ...
        emimPM = lopes.apply(cation, residues = ['emim'])
        anionPM = lopes_fluor.apply(anion, residues= [job.statepoint()['anion']])
 
        systemPM = emimPM + anionPM       
